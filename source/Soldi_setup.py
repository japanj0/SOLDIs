...
def YA():
    show_loading_screen("Подготовка АРКЕИД")
    def ya_thread():
        time.sleep(2)
        win.after(0, lambda: [win.destroy(), UnitedBrowsersModul.main("arcade")])
    threading.Thread(target=ya_thread, daemon=True).start()
def Chrome():
...
def create_main_interface():
    ...
    for i in range(4):
        buttons_frame.grid_columnconfigure(i, weight=1, uniform="browsers")
    ...
    button_yandex = create_browser_button(buttons_frame, "АРКЕИД", ya_im, YA)

    button_firefox.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
    button_edge.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
    button_chrome.grid(row=0, column=2, padx=10, pady=10, sticky="nsew")
    button_yandex.grid(row=0, column=3, padx=10, pady=10, sticky="nsew")
...
def main():
    global win, title_font, button_font_large, exit_font
    global ed_im, fox_im, chrome_im, ya_im
    ...
    try:
        ...
        ya_im = ImageTk.PhotoImage(Image.open(ya_temp).resize(img_size)) if ya_temp else None
    except Exception:
        img_size = (90, 90)
